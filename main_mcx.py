...
# ✅ Google Sheet Auth for MCX
scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
creds = Credentials.from_service_account_file("credentials_mcx.json", scopes=scope)
client = gspread.authorize(creds)

# ✅ MCX Sheet
sheet_id = os.getenv("MCX_SHEET_ID")
sheet_name = os.getenv("MCX_SHEET_NAME", "AutoSignal")
...
if not session.get("access_token"):
    print("❌ MCX Login Failed")
    exit()

print("✅ Logged in for MCX")
...
for i, row in df.iterrows():
    try:
        ...
        print(f"MCX → {row.get('Symbol')} → Final Signal: {final}")
