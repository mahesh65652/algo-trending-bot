...
def calculate_indicators(df):
    try:
        ...
        # PCR Calculation
        # NOTE: यह भाग तभी काम करेगा जब 'Put_Volume' और 'Call_Volume' डेटा Google Sheet में उपलब्ध हो।
...
if __name__ == "__main__":
    ...
    if not all(col in df_updated_sheet.columns for col in required_sheet_cols):
        error_msg = f"❌ Error: Missing required columns in Google Sheet: {', '.join(col for col in required_sheet_cols if col not in df_updated_sheet.columns)}. Please ensure all required headers exist."
    ...
    # Merge Google Sheet data (especially volume for PCR) with historical data
