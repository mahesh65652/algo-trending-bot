...
# ✅ FIX: Updated to filter by both 'symbol' and 'name'
def get_tokens_from_api():
    try:
        ...
        # Check for indices by name or symbol and specific instrument types
        nifty_token_info = df[
            (df['exch_seg'] == 'NSE') & 
            ((df['name'] == 'NIFTY 50') | (df['symbol'] == 'NIFTY'))
        ]
        banknifty_token_info = df[
            (df['exch_seg'] == 'NSE') & 
            ((df['name'] == 'NIFTY BANK') | (df['symbol'] == 'BANKNIFTY'))
        ]
        
        # Combine the dataframes
        fo_indices = pd.concat([nifty_token_info, banknifty_token_info]).drop_duplicates(subset=['symboltoken'])

        if fo_indices.empty:
            logging.error("❌ Filtered dataframe is empty. No tokens found.")
            return {}

        tokens_dict = fo_indices.set_index("symbol").to_dict("index")
