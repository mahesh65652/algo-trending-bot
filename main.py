...
def fetch_and_save_tokens():
    try:
        ...
        filtered_df = df[
            ...
            (df['exch_seg'] == 'NSE') & df['name'].str.contains(indices_pattern, regex=True, na=False)
        ].copy()
...
def get_tokens():
    try:
        ...
        for symbol, item in data.items():
            if 'NIFTY 50' in item.get('name', '').upper():
                tokens_dict['NIFTY'] = item
            elif 'NIFTY BANK' in item.get('name', '').upper():
                tokens_dict['BANKNIFTY'] = item
            elif 'NIFTY FINANCIAL SERVICES' in item.get('name', '').upper():
                tokens_dict['FINNIFTY'] = item
            elif 'NIFTY MIDCAP 100' in item.get('name', '').upper():
                tokens_dict['MIDCPNIFTY'] = item
            elif 'SENSEX' in item.get('name', '').upper():
                tokens_dict['SENSEX'] = item
            else:
        ...
        logging.info(f"Found FINNIFTY token: {tokens_dict.get('FINNIFTY')}")
...
if __name__ == "__main__":
    ...
    # This is the single, combined function call now
    tokens = get_tokens()
