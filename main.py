...
# ✅ FIX: Updated function to fetch historical data more reliably
def fetch_historical_data(api, symbol, token_info, days=15):
    try:
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)

        # Use NFO for NIFTY/BANKNIFTY indices
        exchange = 'NFO' if symbol in ['NIFTY', 'BANKNIFTY'] else token_info.get('exch_seg')
...
def calculate_indicators(df):
    try:
        ...
        if len(df) < 26:  # Need at least 26 data points for MACD calculation
...
if __name__ == "__main__":
    ...
    # ✅ FIX: Get specific tokens for NIFTY and BANKNIFTY
    nifty_token = tokens.get('NIFTY')
    banknifty_token = tokens.get('BANKNIFTY')
    
    if not nifty_token or not banknifty_token:
        logging.error("Failed to get tokens for NIFTY or BANKNIFTY. Cannot proceed.")
        update_google_sheet_cell(gs_client, GSHEET_ID, SHEET_NAME, "G2", "⚠️ NIFTY/BANKNIFTY tokens not found")
        exit(1)
    ...
    # ✅ FIX: Use the fetched tokens to fetch historical data
    full_df = pd.DataFrame()
    for symbol in df['SYMBOL'].unique():
        token_info = tokens.get(symbol)
        if token_info:
            hist_df = fetch_historical_data(angel_api, symbol, token_info, days=30) # Fetch 30 days of data
    ...
    if signals:
        ...
        for s in signals:
            ...
            if info:
                try:
                    quantity_from_sheet = df[df['SYMBOL'] == symbol]['QUANTITY'].iloc[0]
