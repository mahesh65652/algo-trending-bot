...
# ✅ FIX: Updated function to get tokens more reliably for both F&O and Indices
def get_tokens_from_api():
    try:
        ...
        # Filter for Options and Indices
        fo_indices = df[
            ((df["exch_seg"] == "NFO") & (df["instrumenttype"] == "OPTIDX")) |
            ((df["exch_seg"] == "NSE") & (df["instrumenttype"] == "INDICES"))
        ].copy()
        ...
        fo_indices = fo_indices.sort_values(["exch_seg", "symbol"])
        return fo_indices.set_index("symbol").to_dict("index")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # ✅ FIX: Check if the required tokens are found
