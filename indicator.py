...
def get_live_prices_and_update_sheet(api, symbols_df, gs_client, sheet_id, sheet_name):
    ...
    if prices_to_update:
        try:
            # ✅ Fix: Update the correct column. The code assumes 'B' is for CLOSE.
...
# ✅ बदलाव: डेटा को अधिक मजबूत तरीके से साफ करने के लिए 'calculate_indicators' फ़ंक्शन को अपडेट किया गया
def calculate_indicators(df):
    try:
        if 'CLOSE' not in df.columns or 'SYMBOL' not in df.columns:
            logging.error("Required columns 'CLOSE' or 'SYMBOL' are missing for indicator calculation.")
            return pd.DataFrame()
        
        # ✅ FIX: 'CLOSE' कॉलम से अतिरिक्त स्पेस हटा दें और इसे संख्या में बदलें।
...
if __name__ == "__main__":
    ...
    # ✅ FIX: यदि संकेतक गणना असफल होती है तो यहीं रुकें
    if df_with_indicators.empty:
        update_google_sheet_cell(gs_client, GSHEET_ID, SHEET_NAME, "G2", "❌ Indicator calculation failed. Check CLOSE column.")
        logging.error("Exiting due to failed indicator calculation.")
        send_telegram_message("❌ Error: Indicator calculation failed. Please check your Google Sheet data format.")
        exit(1)
